...
class ScrabbleField(val matrix: Matrix, val dictionary: Dictionary, squareFactory: StandardSquareFactory, languageEnum: LanguageEnum):
  ...
  def addSpace(numSpaceToAdd: Int): String = numSpaceToAdd match
    case n if n <= 0 => " "
    case n => " " + addSpace(n - 1)
  def placeWord(yPosition: Int, xPosition : Int, direction :Char, word : String): ScrabbleField = new ScrabbleField(matrix.placeWord(yPosition, xPosition, direction, word), dictionary, squareFactory, languageEnum, player, players)
